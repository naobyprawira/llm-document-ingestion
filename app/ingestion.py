...
3. For pages without flowcharts, extracts the text and chunks it
   ...
   falls back to a smart chunking algorithm.
...
from .utils_pdf import open_pdf, extract_page_text
from .chunking_structure import chunk_by_heading
from .chunking_smart import smart_chunk
from .vlm_flowchart import detect_flowchart, extract_flowchart_steps
...
def ingest_document(
    ...
    security_class: str
        Security classification (e.g. internal, confidential). Unused by the pipeline but stored as metadata.
    ...
    for page_index in range(doc.page_count):
        ...
        if len(chunks) == 1 and (not chunks[0][0]):
            smart_chunks = smart_chunk(page_text, max_tokens=conf.max_tokens_per_chunk)
